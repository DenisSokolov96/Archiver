...
def compress_data(chunks_list, options, file_path):
    ...
    File.write_bytes(file_path + options['postfix_file'],
                     codes_table_serialize_len + codes_table_serialize + compressed_bytes)
    return True


def decompress_data(options, file_path):
    """Основная логика выполнения алгоритма распаковки файла"""
    codes_table_serialize, compressed_data = File.read_compress_file(file_path)
